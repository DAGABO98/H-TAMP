...
class SequentialGreedyBasePolicy:
    def __init__(self):
        self.real_requests_queue = TaskQueue()
    ...
    def _add_all_real_requests_to_queues(self, 
        ...
        if real_requests_lists is None:
            return None
        pickup_deadlines = []
        for data_field in real_requests_lists.__dataclass_fields__.keys():
            requests_list = getattr(real_requests_lists, data_field)
            for request in requests_list:
                pickup_deadline = PolicyHelpers._add_request_to_queue(request=request,
                                           task_queue=self.real_requests_queue,
    ...
    def _assign_request_to_robot(self,
        ...
        if len(potential_assignments) == 0:
            print(f"No potential assignments found for request {request_id}. Request is rejected.")
            ...
            if planned_path:
                ...
                print(f"Failed to find a valid path for the only potential assignment of request {request_id} to robot {robot_id}. Request is rejected.")
                request_struct = state.requests[request_id]
            ...
            if min_planned_path:
                ...
                if debug:
                    ...
                    print(f"2) Planned path for assignment of request {request_id} to robot {min_robot_id}: {min_planned_path}")
                ...
                print(f"Assigned request {request_id} to robot {min_robot_id}")
            else:
    ...
    def _assign_requests_to_robots(self,
                                  ...
                                  traversal_graph_generator: TraversalGraphGenerator,
                                  debug: bool):
        while self.real_requests_queue.heap:
            next_request_id = self.real_requests_queue.pop_task()
            self._assign_request_to_robot(request_id=next_request_id,
                                          state=state,
                                          motion_planner=motion_planner,
                                          traversal_graph_generator=traversal_graph_generator,
                                          debug=debug)
        
        while self.predicted_requests_queue.heap:
            next_request_id = self.predicted_requests_queue.pop_task()
            self._assign_request_to_robot(request_id=next_request_id,
                                          state=state,
                                          motion_planner=motion_planner,
                                          traversal_graph_generator=traversal_graph_generator,
    ...
    def assign_requests_to_robots(self, 
        ...
        smallest_real_pickup_deadline = self._add_all_real_requests_to_queues(requests_lists=real_requests_lists,
                                                                              motion_planner=motion_planner,
                                                                              traversal_graph_generator=traversal_graph_generator)
        
        smallest_predicted_pickup_deadline = self._add_all_predicted_requests_to_queues(requests_lists=predicted_requests_lists,
                                                                                      motion_planner=motion_planner,
                                                                                      traversal_graph_generator=traversal_graph_generator)
        if smallest_real_pickup_deadline and smallest_predicted_pickup_deadline:
            smallest_pickup_deadline = min(smallest_real_pickup_deadline, smallest_predicted_pickup_deadline)
        elif smallest_real_pickup_deadline:
            smallest_pickup_deadline = smallest_real_pickup_deadline
        elif smallest_predicted_pickup_deadline:
            smallest_pickup_deadline = smallest_predicted_pickup_deadline
        else:
            smallest_pickup_deadline = None
