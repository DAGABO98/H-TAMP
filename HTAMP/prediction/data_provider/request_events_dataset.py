...
def _requests_dataset_config_snapshot(
    dataset_config: RequestEventDatasetConfig,
) -> dict[str, object]:
    ...
    return _json_safe_value(dataset_payload)
...
def _requests_time_series_signature(
    ...
    dataset_payload = _requests_dataset_config_snapshot(dataset_config=dataset_config)
...
class RequestsDataManager:
    ...
    def _build_task_events(
        ...
        event_df[FLOOR_COLUMN] = self._floor_series(task_df)
    ...
    def _build_metadata(self) -> dict[str, object]:
        return {
            "version": REQUESTS_DATASET_VERSION,
            ...
            "config_snapshot": _requests_dataset_config_snapshot(self.dataset_config),
        }
    ...
    def _load_split(self, split_name: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if not data_df.empty:
            ...
            if FLOOR_COLUMN not in data_df.columns:
                data_df[FLOOR_COLUMN] = pd.Series(pd.NA, index=data_df.index, dtype="Int64")
            else:
                data_df[FLOOR_COLUMN] = _normalize_floor_series(data_df[FLOOR_COLUMN])
    ...
    def _load_dataframes(self) -> None:
        ...
        if int(self.metadata.get("version", -1)) != REQUESTS_DATASET_VERSION:
            raise ValueError("Saved monitoring-request split data version mismatch.")
        if self.metadata.get("config_snapshot") != _requests_dataset_config_snapshot(self.dataset_config):
            raise ValueError(
                "Saved monitoring-request split data does not match the current dataset configuration."
            )
...
class MonitoringRequestsTimeSeries:
    ...
    def save_cache(self, dataset_config: RequestEventDatasetConfig) -> Path:
...
RequestEventsTimeSeries = MonitoringRequestsTimeSeries
RequestEventsDataset = MonitoringRequestsDataset
...
class RequestEventTimeSeriesBuildConfig:
    def __init__(
        self,
        *,
        seq_len: int = 5,
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "RequestEventTimeSeriesBuildConfig":
        return cls(
            seq_len=int(payload.get("seq_len", 5)),
            label_len=int(payload.get("label_len", 0)),
            pred_len=int(payload.get("pred_len", 3)),
            batch_size=int(payload.get("batch_size", 32)),
            num_workers=int(payload.get("num_workers", 0)),
        )

    def sync_channel_dimensions(
        self,
        *,
        num_input_channels: int,
        num_output_channels: int,
    ) -> None:
        self.enc_in = int(num_input_channels)
        self.c_out = int(num_output_channels)
...
def main() -> int:
    ...
    try:
        ...
        with Path(args.config_path).open("r", encoding="utf-8") as config_file:
            config_payload = json.load(config_file)

        if not isinstance(config_payload, dict):
            raise ValueError("Expected config file to contain a JSON object.")
        dataset_config = RequestEventDatasetConfig.from_dict(
            config_payload.get("dataset_config", config_payload)
        )
        raw_model_config = config_payload.get("model_config", {})
        if not isinstance(raw_model_config, dict):
            raise ValueError("Expected 'model_config' to be a JSON object when provided.")
        model_config = RequestEventTimeSeriesBuildConfig.from_dict(raw_model_config)
        ...
        q_stop = datetime.datetime.now()
        print("Execution time: " + str(q_stop - p_start))
